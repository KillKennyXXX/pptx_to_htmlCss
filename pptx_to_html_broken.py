...
class PPTXToHTMLConverter:
    ...
    def process_slide(self, slide, slide_num):
        ...
        def process_shape_recursive(shape, level=0):
            ...
            nonlocal img_counter, shapes_data
            
            # Пропускаем FREEFORM фигуры, которые использованы как фон слайда
            if shape.shape_type == MSO_SHAPE_TYPE.FREEFORM and background:
                try:
                    # Проверяем, это ли фоновая фигура
                    shape_area = self.emu_to_px(shape.width) * self.emu_to_px(shape.height)
                    slide_area = slide_width * slide_height
                    area_percent = (shape_area / slide_area) * 100
            ...
            if shape.shape_type == MSO_SHAPE_TYPE.GROUP:
                ...
                group_data = {
                    'type': 'group',
                    'style': {},
                    'content': [],  # Список дочерних фигур
                    'group_bounds': {}
                }
                
                try:
                    # Получаем стиль группы (позиция и размер контейнера)
                    group_data['style'] = self.extract_shape_style(shape, slide_width, slide_height)
                    
                    # Сохраняем абсолютные координаты группы для пересчета дочерних элементов
                    group_data['group_bounds'] = {
                        'left': shape.left,
                        'top': shape.top,
                        'width': shape.width,
                        'height': shape.height
                    ...
                    # Создаем отдельную функцию для обработки дочерних фигур группы
                    def process_group_children(parent_shape, level):
                        """Обрабатывает дочерние фигуры группы"""
                        group_children = []
                        
                        for sub_shape in parent_shape.shapes:
                            child_data = process_single_shape(sub_shape, level + 1)
                            if child_data:
                                if isinstance(child_data, list):
                                    group_children.extend(child_data)
                                else:
                                    group_children.append(child_data)
                        
                        return group_children
                    
                    group_data['content'] = process_group_children(shape, level)
                    shapes_data.append(group_data)
                    
                except Exception as e:
                    print(f"  Предупреждение: ошибка обработки группы: {e}")
                    # В случае ошибки обрабатываем как раньше
                    for sub_shape in shape.shapes:
                        process_shape_recursive(sub_shape, level + 1)
                
                return
            
            # Обрабатываем одиночную фигуру
            result = process_single_shape(shape, level)
            if result:
                if isinstance(result, list):
                    shapes_data.extend(result)
                else:
                    shapes_data.append(result)
        
        def process_single_shape(shape, level=0):
            """Обрабатывает одну фигуру и возвращает её данные"""
